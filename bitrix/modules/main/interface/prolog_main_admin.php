...
<head>
    ...
    <title><? $adminPage->ShowTitle() ?>
        - <?= htmlspecialcharsbx(COption::GetOptionString("main", "site_name", $_SERVER["SERVER_NAME"])) ?></title>
    <?
    else:
        ?>
    ...
    <tr>
        ...
        <? if (!$isSidePanel): ?>
            <td class="adm-left-side-wrap" id="menu_mirrors_cont">
                ...
                <div class="adm-left-side<?= $bOptMenuMinimized ? ' adm-left-side-wrap-close' : '' ?>"<? if (isset($aOptMenuPos["width"]) && intval($aOptMenuPos["width"]) > 0) echo ' style="width:' . ($bOptMenuMinimized ? 15 : intval($aOptMenuPos["width"])) . 'px" data-width="' . intval($aOptMenuPos["width"]) . '"' ?>
                     id="bx_menu_panel">
                ...
                if ($USER->IsAuthorized()):
                    ...
                    <?= $sSupportMess; ?>
                        <div id="supdescr" style="display: none;">
                            ...
                            <hr><?= GetMessage("prolog_main_support_wit_description" . (IsModuleInstalled("intranet") ? "_cp" : "_bus"), ['#LINK#' => $eulaLink]) ?>
                        </div>
                        <?
                        echo EndNote();
                    }
